...
productsRoute.post("/", (0, express_async_handler_1.default)(async (req, res) => {
    const data = await productsController.createProduct(req.body);
    res.status(201).json({ messsage: "Product created successfully", data });
}));
productsRoute.get("/:productId", (0, celebrate_1.celebrate)({ params: validation_schema_1.getSingleProductSchema }), (0, express_async_handler_1.default)(async (req, res) => {
    const data = await productsController.getProductById(req.params.productId);
    res.status(200).json(Object.assign({}, data));
}));
productsRoute.put("/:productId", (0, celebrate_1.celebrate)({ params: validation_schema_1.getSingleProductSchema }), (0, express_async_handler_1.default)(async (req, res) => {
    const data = await productsController.updateProduct(req.params.id, req.body);
    res.status(200).json(Object.assign({}, data));
}));
productsRoute.delete("/:productId", (0, celebrate_1.celebrate)({ params: validation_schema_1.getSingleProductSchema }), (0, express_async_handler_1.default)(async (req, res) => {
